    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                console.error(`API Error Response (Status ${response.status}):`, errorBody.substring(0, 500)); // Логгируем начало текста ошибки
                ...
                try {
                    ...
                    errorData = { error: `Server returned status ${response.status}. Response body: ${errorBody.substring(0, 200)}...` };
                }

            } catch (readError) {
                // Если даже чтение как текст не удалось (редко, но возможно)
                console.error(`API Error (Status ${response.status}): Could not read error response body.`, readError);
                errorData = { error: `HTTP error! status: ${response.status}. Failed to read response body.`};
            }
        ...
        try {
            ...
            console.error(`API Error: Status ${response.status} OK, but failed to parse JSON response body.`, jsonError);
            ...
            throw new Error(`Server returned status ${response.status}, but the response body was not valid JSON.`);
        }
...
export const uploadCourse = (formData) => {
    ...
    const courseId = formData.get('courseId') || `${encodeURIComponent(courseName)}-${Date.now()}`; // Используем ID из FormData, если есть
    return Promise.resolve({ id: courseId, name: courseName, message: 'Курс (имитация) успешно загружен' });
};
