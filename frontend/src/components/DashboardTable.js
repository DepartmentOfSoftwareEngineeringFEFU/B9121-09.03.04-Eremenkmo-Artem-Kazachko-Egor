...
function DashboardTable({
  ...
  const columns = [
    ...
    { field: "step_id", headerName: "ID", width: 80, type: "number" },
    {
      ...
      renderCell: (params) => {
        const stepName =
          params.row.step_title_full ||
          params.row.step_title_short ||
          `Шаг ${params.row.step_id}`;
        const overallStatus = getStepOverallStatus(
          params.row,
          availableMetrics
        );
    ...
    { field: "step_title_short", headerName: "Метка", width: 150 },
    {
      ...
      valueGetter: (_value, row) => {
        ...
        return (
          module?.title || (row.module_id ? `ID: ${row.module_id}` : "N/A")
        );
      },
    },
    ...availableMetrics.map((metric) => ({
      ...
      type: "number",
      width:
        metric.label.includes("время") ||
        metric.label.includes("Название") ||
        metric.label.includes("комментар")
          ? 140
          : 115,
    ...
    })),
    {
      ...
      renderCell: (params) => (
        <Button
          ...
          to={
            courseIdForLocalStorage
              ? `/step/${params.row.step_id}?courseId=${encodeURIComponent(
                  courseIdForLocalStorage
                )}`
              : "#"
          }
      ...
      <Paper sx={{ height: 600, width: "100%", mb: 2 }}>
        <DataGrid
          ...
          sx={{
            ...
            "& .MuiDataGrid-cell.metric-text--good .MuiDataGrid-cellContent, & .MuiDataGrid-cell .metric-text--good":
              {
                // Целимся в контент ячейки ИЛИ в span внутри
                color: "green",
                fontWeight: 500,
              },
