...
import { getCourseCompletionRate /*, getAllStepMetricsForCourse */ } from '../api/apiService';
...
function Dashboard() {
    ...
    const [metricsData, setMetricsData] = useState({
        ...
        global: { // Объект с глобальными метриками
            completionRate: null,
            usersCompleted: null,
            totalLearners: null,
    ...
    useEffect(() => {
        ...
        if (idFromUrlDecoded) {
            ...
            if (storedCourses) {
                try {
                    ...
                    if (currentCourse) {
                        ...
                        console.warn(`Курс с ID "${idFromUrlDecoded}" не найден в localStorage.`);
                    }
                } catch (e) {
            ...
            const fetchData = async () => {
                try {
                     console.log(`Dashboard: Загрузка данных для курса ID (декодир.): ${idFromUrlDecoded}`);
                    // Параллельно запрашиваем общую статистику и статистику по шагам
                    const [completionData, stepsData] = await Promise.all([
                        getCourseCompletionRate(), // <--- Реальный вызов API
                        // getAllStepMetricsForCourse(idFromUrlDecoded) // <--- TODO: Заменить на реальный вызов API, когда будет готов эндпоинт
                      
                        // ВРЕМЕННО: Используем mock-данные для шагов
                        Promise.resolve(generateMockStepData(mockMetrics)),
                    ]);

                     console.log("Dashboard: Получены данные Completion Rate:", completionData);
                    // console.log("Dashboard: Получены данные по шагам (пока мок):", stepsData);
                    console.log("<<< ДАННЫЕ С БЭКЕНДА (completion_rate):", completionData);
                    console.log("<<< ДАННЫЕ ПО ШАГАМ (mock):", stepsData);
                    // Готовим обновление для глобальных метрик
                    const globalMetricsUpdate = {
                        completionRate: completionData.course_completion_rate,
                        usersCompleted: completionData.users_completed,
                        totalLearners: completionData.total_learners,
                        // Здесь можно будет добавить другие глобальные метрики из API, если они появятся
                    };

                    // Обновляем состояние
                    setMetricsData((prev) => ({
                        steps: stepsData, // Заменяем данные по шагам (пока моками)
                        global: { ...prev.global, ...globalMetricsUpdate }, // Обновляем часть глобальных метрик
                    }));

                } catch (err) {
                    console.error('Ошибка загрузки данных дашборда:', err);
                    setError(err.message || 'Не удалось загрузить данные для дашборда.');
                    setMetricsData({ steps: [], global: {} }); // Сброс данных при ошибке
                } finally {
                    setLoading(false);
                }
            };
    ...
    const handleCompareSteps = () => {
        ...
        const storedCourses = localStorage.getItem('uploadedCourses');
         if (storedCourses && courseId) { // courseId здесь - декодированный
            try {
                const courses = JSON.parse(storedCourses);
                const currentCourse = courses.find(c => decodeURIComponent(c.id) === courseId);
                if (currentCourse) {
    ...
    const columns = [
        {
            field: 'selection', headerName: 'Выбор', width: 80, sortable: false,
            renderCell: (params) => (
                <FormControlLabel sx={{ mr: 0 }} control={
                        <Checkbox size="small" checked={selectedStepIds.includes(params.row.step_id)}
        ...
        { field: 'self_correction_rate', headerName: 'Самокоррекция (%)', width: 130 }, // Убедитесь, что данные приходят в % или обрабатываются
        {
            field: 'actions', headerName: 'Анализ', sortable: false, width: 120,
            renderCell: (params) => {
                ...
                const storedCourses = localStorage.getItem('uploadedCourses');
                 ...
                 if (!encodedCourseIdForURL && courseId) encodedCourseIdForURL = encodeURIComponent(courseId);

                 return (
                     <Button variant="outlined" size="small" component={RouterLink} disabled={!encodedCourseIdForURL}
                ...
                <Grid container spacing={3} sx={{ mb: 3 }}>
                    ...
                    <Grid item xs={12} sm={4}>
                        <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                            <Typography component="h2" variant="h6" color="primary" gutterBottom>Завершили курс</Typography>
                            <Typography component="p" variant="h4">
                                {globalMetrics.completionRate !== null
                                    ? `${(globalMetrics.completionRate * 100).toFixed(1)}%`
                                    : 'N/A'}
                            </Typography>
                            {globalMetrics.usersCompleted !== null && globalMetrics.totalLearners !== null && (
                                <Typography variant="caption" color="textSecondary">
                                    ({globalMetrics.usersCompleted} из {globalMetrics.totalLearners})
                                </Typography>
                            )}
                        </Paper>
                    </Grid>
                    {/* Карточка "Средний % завершения" (пока мок) */}
                     <Grid item xs={12} sm={4}>
                          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                               <Typography component="h2" variant="h6" color="primary" gutterBottom>Средний % завершения</Typography>
                               <Typography component="p" variant="h4">
                                    {globalMetrics.avg_completion_percent !== undefined ? `${(globalMetrics.avg_completion_percent * 100).toFixed(1)}%` : "N/A"}
                               </Typography>
                          </Paper>
                     </Grid>
                     ...
                     <Grid item xs={12} sm={4}>
                          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                               <Typography component="h2" variant="h6" color="primary" gutterBottom>Среднее время м/у шагами</Typography>
                               <Typography component="p" variant="h4">
                                    {globalMetrics.avg_time_between_steps !== undefined ? `${globalMetrics.avg_time_between_steps} сек` : "N/A"}
                               </Typography>
                          </Paper>
                     </Grid>
                </Grid>
            ...
            {stepData.length > 0 ? (
                <Paper sx={{ p: 2, mb: 3 }}>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                        <LineChart data={dataForChart} margin={{ top: 5, right: 30, left: 20, bottom: 70 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="step" angle={-45} textAnchor="end" height={80} interval={0} tick={false} />
                            <YAxis />
                            <Tooltip />
                            <Legend />
                            <Line type="monotone" dataKey="value" name={selectedMetric.label} stroke="#8884d8" activeDot={{ r: 8 }} />
                            {minValue !== Infinity && minValue !== 0 && ( // Не показываем для 0
                                <ReferenceLine y={minValue} label={{ value: `Min: ${minValue}`, position: 'insideTopLeft' }} stroke="red" strokeDasharray="3 3" />
                            )}
                            {maxValue !== -Infinity && maxValue !== 0 && ( // Не показываем для 0
                                <ReferenceLine y={maxValue} label={{ value: `Max: ${maxValue}`, position: 'insideTopRight' }} stroke="green" strokeDasharray="3 3" />
                            )}
                        </LineChart>
                    </ResponsiveContainer>
                </Paper>
