...
const formatNumber = (value) => {
    if (typeof value !== 'number') return 'N/A';
    return value.toFixed(1); // Например, для попыток
}
    ...
    useEffect(() => {
        ...
        const fetchStepData = async () => {
            ...
            try {
                ...
                console.error(`StepAnalysis: Ошибка загрузки данных для шага ${stepId}:`, err);
        ...
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
                ...
                <Typography variant="h4">
                    Анализ шага: {stepMetrics.stepId}
                </Typography>
            </Box>
            ...
            <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 3, height: '100%' }}>
                        <Typography variant="h6" gutterBottom color="primary">Прохождение</Typography>
                        <Typography>
                            <b>Успешно прошли:</b> {formatInteger(stepMetrics.usersPassed)} из {formatInteger(stepMetrics.allUsers)} пользователей
                        </Typography>
                        <Typography>
                            <b>Результативность (Прошли / Приступили):</b> {formatPercentage(stepMetrics.effectiveness)}
                        </Typography>
                        <Typography>
                            <b>Среднее время до верного ответа:</b> {formatSeconds(stepMetrics.avgCompletionTime)}
                        </Typography>
                    </Paper>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 3, height: '100%' }}>
                         <Typography variant="h6" gutterBottom color="primary">Попытки и Взаимодействие</Typography>
                        <Typography>
                            <b>Успешность попыток (Верные / Все):</b> {formatPercentage(stepMetrics.successRate)}
                        </Typography>
                        <Typography>
                            <b>Среднее число попыток на прошедшего:</b> {formatNumber(stepMetrics.avgAttempts)}
                        </Typography>
                         <Typography>
                            <b>Количество комментариев:</b> {formatInteger(stepMetrics.commentsCount)}
                        </Typography>
                    </Paper>
                </Grid>
                 ...
                 <Grid item xs={12}>
                     <Paper sx={{ p: 2, mt: 2, backgroundColor: '#f5f5f5' }}>
                        <Typography variant="h6" gutterBottom>Рекомендации (Заглушка)</Typography>
                         <Typography paragraph>
                             Здесь будут отображаться автоматические рекомендации на основе анализа метрик (например, если результативность низкая или время большое). Пока не реализовано.
                         </Typography>
                     </Paper>
                 </Grid>
            </Grid>
